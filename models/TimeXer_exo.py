...
from models.TimeXer import Model as TimeXerBackbone
...
class Model(nn.Module):
    """STIC-style exogenous wrapper around TimeXer."""

    def __init__(self, configs: Any) -> None:
        super().__init__()
        if configs.task_name not in {"long_term_forecast", "short_term_forecast"}:
            raise NotImplementedError(
                "TimeXer_exo currently supports only forecasting tasks."
            )
        ...
        self.history_branch = TimeXerBackbone(history_configs)
        self.context_branch = TimeXerBackbone(context_configs)
    ...
    def forecast(
        ...
        if has_context:
            ...
            pred_c = self.context_branch(context_enc, x_mark_enc, context_dec, x_mark_dec)

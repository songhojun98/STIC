...
from models.iTransformer import Model as ITransformerBackbone


class Model(nn.Module):
    """STIC-style exogenous wrapper around iTransformer."""

    def __init__(self, configs: Any) -> None:
        super().__init__()
        if configs.task_name not in {"long_term_forecast", "short_term_forecast"}:
            raise NotImplementedError(
                "iTransformer_exo currently supports only forecasting tasks."
            )
        ...
        self.history_branch = ITransformerBackbone(history_configs)
        self.context_branch = ITransformerBackbone(context_configs)
